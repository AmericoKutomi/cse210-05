...
class GreenSnake(Snake):
    """
    A Snake green colored
    
    A Green Snake is derived from Snake class
    ...
    def grow_tail(self, number_of_segments):
        ...
        for i in range(number_of_segments):
            ...
            segment.set_text("#")
            segment.set_color(constants.GREEN)          # a specific color is assigned to the Snake
            self._segments.append(segment)

    def _prepare_body(self):
        # the inherited _prepare_body is NOT called because it is totally overridden here

        x = int(constants.MAX_X / 6 * 5)                # the specific position of the Snake is defined here
